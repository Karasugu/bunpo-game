...
label stage1_5:
    ...
    $ explanation = "「どう見ても」という表現はおかしいです。「〜ても」の文型では、前半の条件が後半の結果に{color=#ff0000}関係{/color}している必要があります。料理の見た目と味は直接関係がないので、この文は不自然です。"
    $ sentence_index = 0
...
label stage2_5:
    ...
    $ explanation = "「〜たびに」の文型では、後半は{color=#ff0000}自然{/color}と起こる結果や感情である必要があります。テレビを見るのは意識的な行動なので、この文は不自然です。"
    $ sentence_index = 0
    call screen stage_screen(sentences, correct, current_stage)
...
label stage3_5:
    ...
    $ explanation = "「日本語が話せるはずです」という表現はおかしいです。「はずです」は、他のことについて{color=#ff0000}推測 (すいそく){/color}をするときに使います。しかし、自分自身のことは100％わかるはずなので、自分のことに「はずです」を使うのは不自然です。"
    $ sentence_index = 0
    call screen stage_screen(sentences, correct, current_stage)
...
label stage4_5:
    ...
    $ explanation = "「友達が帰っておきました」という表現はおかしいです。「〜ておく」は、何かに{color=#ff0000}備えて事前{/color}に準備の行動をすることを表します。しかし「遅くなったので」は結果を表すので、準備の意味と合いません。"
    $ sentence_index = 0
    call screen stage_screen(sentences, correct, current_stage)
...
label stage5_5:
    ...
    $ explanation = "「（もし）〜ても」の文型では、後半は前半の条件から{color=#ff0000}予想外（よそうがい）{/color}のことを表す必要があります。雨の日に家でゲームをするのは普通のことなので、この文はおかしいです。"
    $ sentence_index = 0
    call screen stage_screen(sentences, correct, current_stage)
...
label stage6_5:
    ...
    $ explanation = "「先生の声が聞けるように」という表現はおかしいです。「〜ように」の文型では、前半には話し手の意志で{color=#ff0000}コントロールできない{/color}動詞を使う必要があります。「聞ける」は意志でコントロールできる動詞なので、「聞こえる」を使うべきです。"
    $ sentence_index = 0
    call screen stage_screen(sentences, correct, current_stage)
...
label stage8_5:
    ...
    $ explanation = "「いつも込んでいるだけあって」という表現はおかしいです。「〜だけあって」の文型では、前半には{color=#ff0000}ポジティブ{/color}な評価や特徴が来るのが一般的です。「いつも込んでいる」は客からするとあまり良いことではないので、この文は不自然です。"
    $ sentence_index = 0
    call screen stage_screen(sentences, correct, current_stage)
